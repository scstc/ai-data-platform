...
async def _start_distillation(
    ...
    if (block_msg := _distill_operator_block(all_operators)) is not None:
        return JSONResponse(
            status_code=400, content={"success": False, "message": block_msg}
        )

    # 至少 1 个 selector(否则蒸馏退化成纯过滤,语义不符);成员级配置逐成员检查
    def _has_selector(ops: list) -> bool:
        return any(
    ...
    if body.member_configs:
        no_selector = [
            cfg.member_name
            for cfg in body.member_configs
            if not _has_selector(cfg.operators)
        ]
        if no_selector:
            return JSONResponse(
                status_code=400,
                content={
                    "success": False,
                    "message": (
                        f"成员 {', '.join(no_selector)} 的算子链必须包含至少 1 个 "
                        "selector(如 topk_specified_field_selector)"
                    ),
                },
            )
    elif not _has_selector(body.operators):
        return JSONResponse(
            status_code=400,
            content={
                "success": False,
...
async def get_distillation_report(
    ...
    if version is None:
        ...
        empty = DistillationReport(
            ...
            input_count=0,
            operator_chain=[o["name"] for o in (job.spec or {}).get("operators") or []],
