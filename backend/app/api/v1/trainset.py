...
async def _start_trainset(
    session: AsyncSession, body: TrainsetJobCreate, job: Job | None = None
) -> JSONResponse:
...
@router.post("/trainset/jobs", dependencies=[Depends(require_admin)])
...
async def update_trainset_job(
    job_id: str, body: TrainsetJobCreate, session: SessionDep
) -> JSONResponse:
    ...
    return await _start_trainset(session, body, job=job)


@router.post(
    "/trainset/jobs/{job_id}/rerun", dependencies=[Depends(require_admin)]
)
async def rerun_trainset_job(job_id: str, session: SessionDep) -> JSONResponse:
    """用原 spec 重跑。"""
    job = await session.get(Job, job_id)
    if job is None or job.type != _TRAINSET_TYPE:
        return JSONResponse(
    ...
    return await _start_trainset(session, spec)
