...
# ===========================================================================
# §4.9 S3 原样归档:_ext 取对象原始扩展名(用作归档 DatasetVersion.format)
# ===========================================================================
def test_ext_extracts_original_extension():
    """_ext 取小写、去点的对象扩展名;无扩展名返回空串(归档时兜底 format=bin)。"""
    assert _ext("a/b/pic.PNG") == "png"  # 去路径 + 小写
    assert _ext("clip.MP4") == "mp4"
    assert _ext("raw/2026/data.csv") == "csv"
    assert _ext("notes.jsonl") == "jsonl"
    assert _ext("noext") == ""  # 无扩展名 → 空串
