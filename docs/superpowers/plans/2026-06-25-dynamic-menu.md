# 全菜单统一管理(动态菜单 · B1)实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: superpowers:executing-plans。Steps 用 `- [ ]` 勾选。

**Goal:** 让「菜单管理」管全部菜单,且**侧边栏按角色显示不同菜单**——侧边栏由后端 `menus` 表(经 `getRouters` 按授权)驱动,经 ProLayout `menuDataRender` 注入;菜单管理页对全部菜单做增删改/排序/显隐/按角色授权。

**Architecture(B1「动态菜单」,用户最终选定;曾考虑 B2 patchClientRoutes 但因其重写路由体系、回归面大,而 B1 与 B2 在"按角色控可见"上能力等同,B2 仅多得"UI 改路由 path"边际能力,不值当):**
- **路由**仍由 `config/routes.ts` 静态定义(零路由风险;所有现有页面/编辑器/参数路由/重定向原样工作)。
- **侧边栏菜单**改由后端 `getRouters` 下发的菜单树驱动(已按角色授权裁剪),经 ProLayout `menuDataRender` 注入。菜单管理页编辑 `menus` 表 + 角色授权 ⇒ 侧边栏随之变。

**按角色控菜单的能力(用户核心诉求):** `getRouters` 已实现"只返回当前用户角色被授权的菜单"。超管看全部;普通用户看授权子集;自定义角色(如"标注员")只看勾选项。B1 与 B2 在此能力上等同。

**Tech Stack:** FastAPI/SQLAlchemy/Alembic、Umi Max + @ant-design/pro-components(`menuDataRender`)。
...
- 超管(`role_key=admin`)看全部;**普通用户(role-000002)回填授予全部业务菜单**(保现有"全员可见")。`/ops/security`、`/ops/llm-settings` 当前是 `access:'canAdmin'`,新模型里**只授给超管角色、不授普通用户**(语义保持)。
- 图标:后端存图标字符串;`menuDataRender` 端把字符串映射到 antd 图标(运行时静态 routes 的字符串图标由 Umi 构建期解析,运行时注入需自带映射表——**关键验证点**)。
...
- Modify `frontend/src/services/system/api.ts` — `getRouters` 前端调用 + `RouterNode` 类型。
- `config/routes.ts` **不动**(路由照旧)。
- Tests:`backend/tests/test_system_routers.py` 扩展(普通用户/超管/自定义角色菜单可见集);前端 tsc/biome + 联调冒烟。
...
- [ ] **1.1** 写 0021:`_seed_business_menu_rows()` 返回上述 ~30 行(顶级 parent_id=null,M 目录先建、C 叶子 parent_id 指向所属 M);固定 id(如 `menu-100001`…);`bulk_insert` menus。再 `bulk_insert` role_menus:普通用户(role-000002)← 全部刚种的业务 M/C(**排除** security/llm-settings)。幂等(NOT EXISTS 防重)。
- [ ] **1.2** 开发库 `alembic upgrade head`;校验:`select count(*) from menus`(应 ≈ 26+30=56)、普通用户经 getRouters 可见的业务菜单数。
- [ ] **1.3** 后端测试扩展:`test_system_routers.py` 加——超管 getRouters 含 `/ingest`、`/datasets` 等业务目录;普通用户经 role_menus 也能看到业务菜单;自定义角色只看到勾选项。
- [ ] **1.4 跑测试→通过;提交**
...
- [ ] **3.1** 菜单管理页现在显示全部 ~56 条菜单树;新建/编辑/排序/显隐对侧边栏生效。
- [ ] **3.2** 角色管理「分配菜单」勾选业务菜单给自定义角色 → 该角色登录侧边栏只看到勾选项(按角色控可见 = 用户核心诉求验证)。
- [ ] **3.3** `docs/` 登记:动态菜单架构 + "新增页面"流程(① routes.ts 加路由;② 菜单管理页加菜单行;③ 按需在角色授权)。
...
- **目标覆盖:** 菜单管理管全部菜单 + **按角色控不同用户可见菜单**(用户核心诉求)⇒ Task 1–3 ✓。
- **范围/风险:** 路由不动 ⇒ 现有页面/编辑器/重定向零回归;唯一新风险是 `menuDataRender` 运行时图标渲染(Task 2.4 验证)。比 B2(patchClientRoutes 重写路由)安全得多。
- **保留:** getRouters/require_perm/角色授权机制复用;`hideInMenu` 路由继续 routing-only。
- **权衡已注明:** 新增"页面"仍需改 routes.ts(组件必须存在代码里),B2 也绕不过;B1 已满足核心诉求。

## 执行说明

按 Task 1→3 顺序。Task 1(后端种子)先交付验证;Task 2 是前端核心(联调冒烟为验收);Task 3 收尾按角色控可见。
