...
| `deployment-10.60.1.64.md` | `10.60.1.64` 全新服务器独立部署记录:该机完全无出网/内网互通能力,借道 `192.168.9.195` 离线构建镜像(CPU-only)+ 经本地机中转 `docker save`/`load`;全新 PG+MinIO+算子目录导入,已知限制与后续更新步骤(2026-07-09) |
| `算子模型依赖盘点.md` | 算子模型依赖全量盘点:212 个可见算子中 ~104 个涉模型,按来源分三类(A. API 模型 33 个归 LLM 配置中心 / B. 本地显式模型 43 个归模型仓库 / C. 隐式模型 28 个代码写死);GPU 守门口径 62 个、已知分类偏差(alphanumeric/token_num 偏严、calibrate_query 等偏松)、LLM 配置收口要点(api_endpoint 计量旁路、filter_records 缺口、双模算子豁免决策)(2026-07-10) |
| `数据集可溯源可复现-评估与整改.md` | **数据集可溯源/可复现评估**:针对「查数据集来源任务+任务全部资产、保证可溯源可复现」需求,逐文件核实三条腿(入湖/出湖抽取/加工产版本)现状,判定当前**不完全满足**;五类资产判定矩阵、阻断溯源 4 项(直连/媒体无来源、两跳湖搬运匿名、成员级对应糊掉)+ 阻断复现 4 项(**LLM 模型运行时解析未固化**为最严重)、P0–P2 整改(P0:LLM 配置快照进 spec + 出湖抽取补建 Job)与验收标准;含关键代码位置表(2026-07-09) |
